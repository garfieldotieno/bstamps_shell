...
from api_master.models import  sale, ticket
...
def generate_transaction_card(image, data):
    try:
        ...
        plate_one_half_reverse_template = Image.open(data['item_media_plate_url'])
        ...
        editable_plate_one_half_reverse_template.text(top_anchor, data['heading'], 'black', font=top_font)
        editable_plate_one_half_reverse_template.text(middle_anchor, data['sub_heading'], 'black', font=middle_font)
        ...
        print(f"type for qr_data_hash is {type(data['qr_data_hash'])}")
        print(f"qr_data_hash value is {data['qr_data_hash']}")


        if data['level'] == 0:
            base_bot_db.json().set(data['qr_data_hash'], '$', data)
        else:
            # Calculate the hash of the modified image
            image_hash = calculate_file_hash(temp_save_path)
            data['tx_file_hash'] = image_hash
            base_bot_db.json().set(data['qr_data_hash'], '$', data)
...
def generate_big_card(image, data):
    try:
        ...
        plate_one_full_centered_template = Image.open(data['item_media_plate_url'])
        editable_plate_one_full_centered_template = ImageDraw.Draw(plate_one_full_centered_template)
        editable_plate_one_full_centered_template.text(header_anchor, data['heading'], 'black', font=header_font)
        editable_plate_one_full_centered_template.text(indicator_anchor, data['indicator'], 'black', font=indicator_font)
        ...
        split_list = temp_save_path.split('/')
        ret_url = f'{split_list[5]}/{split_list[6]}/{split_list[7]}'
        ...
        print(f'errorr during generating_big_card_level_0 {response}')
        return response
...
def generate_receipt(image, data):
    try:
        ...
        plate_full = Image.open(data['item_media_plate_url'])
        editable_plate_full_template = ImageDraw.Draw(plate_full)
    
   
        next_service_date_display = data['next_service_date']
    
        editable_plate_full_template.text(header_anchor, data['heading'], 'black', font=header_font)
        editable_plate_full_template.text(sub_header_anchor, data['sub_heading'], 'black', font=sub_header_font)
        editable_plate_full_template.text(service_topic_anchor, data['service_heading'], 'black', font=service_topic_font, anchor='ms')
        ...
        editable_plate_full_template.text(sale_description_anchor, sale_receipt_chunker(20, data['sale_description']), 'black', font=sale_description_font, anchor='ms')
        editable_plate_full_template.text(sale_amount_anchor, str(comma(data['sale_amount'])), 'black', font=sale_amount_font, anchor='ms')

        sale_card_file_name =''
        for i in range(10):
            sale_card_file_name +=  secrets.choice(alphanumerics)

        temp_save_path = f'{staging_path}'+'/'+f'{data["sub_heading"]}:receipt:sale:'+f'{sale_card_file_name}.png'
...
def lookup_hash_key(hash_key):
    res = base_bot_db.json().get(hash_key)
    return res
...
def reference_calculated_hash(hash_value):
    # reference from redis store
    pass