...
class Session():
    def __init__(
        ...
        self.session_type = session_active
    ...
    def get_main_menu_select(waid):
        key=f"session:{waid}"
        return hermes_bot_db.hget(key, "main_menu_select").decode('utf-8')
    ...
    def is_submenu_browsing(waid):
        key=f"session:{waid}"
        sub_1 = hermes_bot_db.hget(key,"sub1_menu_nav").decode('utf-8')
        sub_2 = hermes_bot_db.hget(key, "sub2_menu_nav").decode('utf-8')

        if sub_1 == '1' or sub_2 == '1':
            return 1
        else:
            return 0
    ...
    def get_current_submenu(waid):
        ...
        if sub_1 == '1':
            sub_menu = hermes_bot_db.hget(key, "sub1_menu_select").decode('utf-8')
        
        if sub_2 == "1":
            sub_menu = hermes_bot_db.hget(key, "sub2_menu_select").decode('utf-8')
    ...
    def get_sub1_menu_select(waid):
        key=f"session:{waid}"
        return hermes_bot_db.hget(key, "sub1_menu_select").decode('utf-8')
    ...
    def is_sub1_menu_nav(waid):
        key=f"session:{waid}"
        return hermes_bot_db.hget(key,"sub1_menu_nav").decode('utf-8')
    ...
    def is_sub2_menu_nav(waid):
        key=f"session:{waid}"
        return hermes_bot_db.hget(key,"sub2_menu_nav").decode('utf-8')
